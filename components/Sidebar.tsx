...
const TenantsIcon = () => (
  ...
  >
    <path
      ...
      strokeLinejoin="round"
      strokeWidth={2}
      d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.653-.124-1.282-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.653.124-1.282.356-1.857m0 0a3.001 3.001 0 01-3.712 0M12 15a3 3 0 100-6 3 3 0 000 6z"
    />
  </svg>
);
const PaymentsIcon = () => (
      ...
      <nav className="flex-1 mt-6">
        {navItems.map((item) => (
          <button
            ...
            className={`flex items-center w-full px-4 py-3 text-sm font-medium transition-colors duration-200 ${currentView === item.view
    ...
    <aside className="hidden md:flex flex-col bg-gray-800 text-white w-64">
      <div className="flex flex-col h-full">
        <div className="flex items-center px-4 h-20 border-b border-gray-700">
          <img src="/logo.png" alt="ZenithRent Logo" className="h-10 w-10 mr-3" />
  ...
  const mobileSidebar = (
    <>
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 h-16 bg-gray-800 text-white flex items-center justify-between px-4 z-30 shadow-md">
        <div className="flex items-center">
          <img src="/logo.png" alt="ZenithRent Logo" className="h-8 w-8 mr-2" />
      ...
      {/* Panel */}
      <div
        className={`fixed top-0 left-0 h-full w-64 bg-gray-800 text-white z-50 transform ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"
